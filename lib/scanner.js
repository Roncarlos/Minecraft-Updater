...
import {
  INSTANCES_ROOT, CACHE_PATH, CACHE_VERSION,
...
export async function listInstances() {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      try {
        const raw = await readFile(instFile, 'utf-8');
        const inst = JSON.parse(raw);
        const mcVersion = inst.gameVersion || '';
        const loaderType = inst.baseModLoader?.type;
        const loaderName = loaderType != null ? (LOADER_MAP[loaderType] || '') : '';
        results.push({ name: entry.name, path: instPath, mcVersion, loaderName });
      } catch { /* no minecraftinstance.json or invalid, skip */ }
    }
...
async function loadCache() {
  try {
    ...
    if (data._version !== CACHE_VERSION) {
      console.log(`Cache version mismatch (got ${data._version}, expected ${CACHE_VERSION}) — rebuilding`);
      return {};
    }
    return data;
  } catch {
...
/**
 * Remove cache entries whose checkedAt is older than the configured prune threshold.
 * Runs once at server startup to keep the cache file from growing forever.
 */
export async function pruneCache() {
  ...
  const pruneDays = settings.cache.pruneDays;
  ...
  for (const key of keys) {
    ...
    try {
      if (new Date(entry.checkedAt).getTime() < cutoff) {
        delete cache[key];
        pruned++;
      }
    } catch {
  ...
  if (pruned > 0) {
    ...
    console.log(`Cache pruned: removed ${pruned} stale entries (older than ${pruneDays}d), ${keys.length - pruned} kept`);
  }
}
...
export async function scanConfigRefs(instancePath, allAddons) {
  ...
  const refFiles = new Map();  // addonID -> Map<relPath, Set<lineNum>>
  ...
  for (const dir of configDirs) {
    ...
    for (const filePath of files) {
      ...
      // Build newline offset table for translating match positions to line numbers
      const lower = content.toLowerCase();
      const lineOffsets = [0]; // lineOffsets[i] = char offset where line i+1 starts
      for (let j = 0; j < lower.length; j++) {
        if (lower[j] === '\n') lineOffsets.push(j + 1);
      }

      // Single regex pass over full content
      ...
      while ((m = regex.exec(lower)) !== null) {
        ...
        if (!refCounts.has(referencedAddonId)) {
          ...
          refFiles.set(referencedAddonId, new Map());
        ...
        // Binary search for line number from match offset
        ...
        const fileMap = refFiles.get(referencedAddonId);
        if (!fileMap.has(relPath)) fileMap.set(relPath, new Set());
        fileMap.get(relPath).add(lineNum);
  ...
  // Convert Maps/Sets to { filePath: sortedLineNumbers[] } for JSON serialization
  const refFilesObj = {};
  for (const [id, fileMap] of refFiles) {
    const obj = {};
    for (const [path, lineSet] of fileMap) {
      obj[path] = [...lineSet].sort((a, b) => a - b);
    }
    refFilesObj[id] = obj;
  }
...
export async function runScan(instancePath, options = {}, onProgress = () => {}) {
  const { noCache = false, limit = 0, checkChangelogs = false, useLlm = false, settings = null, signal = null } = options;
  ...
  const maxAge = currentSettings.cache.maxAgeHours * 3600 * 1000;
  ...
  for (let i = 0; i < total; i++) {
    ...
    const refFilesList = Object.keys(refFiles[addonID] || {});
      ...
      if (hasUpdate && cachedCLs && cachedCLs.length > 0) {
        if (useLlm && settings) {
          ...
          if (cached.llmChangelogs && cached.llmChangelogs.length > 0) {
            ...
            try {
              llmChangelogs = await analyzeChangelogs(cachedCLs, modName, settings, null, signal);
    ...
    // Query API
    try {
      const files = await getModFiles(addonID, mcVersion, loaderType, loaderName, signal);
      ...
      if (!downloadUrl) {
        try {
          downloadUrl = await getDownloadUrl(addonID, latest.id, signal);
        } catch { /* not critical */ }
      }
      ...
      if (hasUpdate) {
        ...
        if (checkChangelogs) {
          ...
          for (const file of intermediateFiles) {
            if (signal?.aborted) break;
            try {
              const cl = await getChangelog(addonID, file.id, signal);
              if (!cl) continue;
          ...
          if (!signal?.aborted && !intermediateFiles.some(f => f.id === latest.id)) {
            try {
              const cl = await getChangelog(addonID, latest.id, signal);
              if (cl) {
                changelogHtml = cl;
                allChangelogs.push({
                  fileId: latest.id,
          ...
          if (useLlm && settings && allChangelogs.length > 0) {
            onProgress({ type: 'status', message: `LLM analyzing changelogs for ${modName}...` });
            try {
              llmChangelogs = await analyzeChangelogs(allChangelogs, modName, settings, null, signal);
              cache[cKey].llmChangelogs = llmChangelogs;
