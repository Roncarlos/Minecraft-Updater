...
import { extractSemver, detectVersionBump, hasBreakingKeywords } from './versioning.js';
  ...
  for (let i = 0; i < total; i++) {
    ...
    try {
      ...
      if (hasUpdate) {
        ...
        if (checkChangelogs) {
          try {
            changelogHtml = await getChangelog(addonID, latest.id);
            if (changelogHtml && hasBreakingKeywords(changelogHtml)) {
              isBreaking = true;
              breakingReason = breakingReason
                ? breakingReason + ' + breaking keywords in changelog'
                : 'Breaking keywords found in changelog';
            }
          } catch { /* not critical */ }
        }
      }
