...
import { readFile, writeFile, mkdir, rm, readdir, stat, lstat, realpath, cp, rename } from "fs/promises";
...
const DISABLE_MODS_MAX_PATTERNS = 50;
const DISABLE_MODS_MAX_LENGTH = 200;

const FILE_REPLACEMENTS_MAX_RULES = 50;
const FILE_REPLACEMENTS_MAX_REPLACEMENTS_PER_RULE = 20;
const FILE_REPLACEMENTS_MAX_PATTERN_LENGTH = 200;
const FILE_REPLACEMENTS_MAX_REPLACEMENT_LENGTH = 1000;
const FILE_REPLACEMENTS_ALLOWED_FLAGS = /^[im]*$/;

/**
 * Compile a regex pattern safely, rejecting invalid or potentially dangerous patterns.
 * Returns the compiled RegExp or null if invalid.
 */
function safeRegex(pattern, flags = "", maxLen = DISABLE_MODS_MAX_LENGTH) {
  if (typeof pattern !== "string" || pattern.length > maxLen) return null;
  try {
    const re = new RegExp(pattern, flags);
    // Basic ReDoS heuristic: reject nested quantifiers like (a+)+ or (a*)*
    if (/([+*]|\{\d)[^)]*\)\s*[+*{]/.test(pattern)) return null;
    return re;
  } catch {
    return null;
  }
...
/**
 * Find mod files in instanceModDir that match any of the given regex patterns.
 * Returns array of { fileName, pattern } for each match.
 */
async function findMatchingMods(instanceModDir, patterns) {
  const matches = [];
  const regexes = [];
  for (const p of patterns) {
    const re = safeRegex(p);
    if (re) regexes.push({ pattern: p, re });
  }
  if (regexes.length === 0) return matches;

  let files;
  try {
    files = await readdir(instanceModDir);
  } catch (err) {
...
async function readPresetJson(id) {
  ...
  if (!Array.isArray(data.disableMods)) data.disableMods = [];
  if (!Array.isArray(data.fileReplacements)) data.fileReplacements = [];
  return data;
}
...
export async function updatePreset(id, updates) {
  ...
  // Handle disableMods array update
  if (Array.isArray(updates.disableMods)) {
    const candidates = updates.disableMods.slice(0, DISABLE_MODS_MAX_PATTERNS);
    const valid = [];
    for (const p of candidates) {
      if (safeRegex(p) === null) {
        throw new Error(`Invalid or unsafe regex pattern: ${p}`);
      }
      valid.push(p);
    }
    preset.disableMods = valid;
  }
  // Handle fileReplacements array update
  if (Array.isArray(updates.fileReplacements)) {
    const rules = updates.fileReplacements.slice(0, FILE_REPLACEMENTS_MAX_RULES);
    const validated = [];
    for (const rule of rules) {
      if (!rule || typeof rule.targetPath !== "string" || !rule.targetPath.trim()) {
        throw new Error("Each file replacement rule must have a non-empty targetPath");
      }
      // Path traversal check — use relative() which is platform-safe
      const normalized = relative(".", join(".", rule.targetPath));
      if (normalized.startsWith("..") || isAbsolute(normalized)) {
        throw new Error(`Invalid targetPath (path traversal): ${rule.targetPath}`);
      }
      if (!Array.isArray(rule.replacements) || rule.replacements.length === 0) {
        throw new Error(`Rule for "${rule.targetPath}" must have at least one replacement`);
      }
      if (rule.replacements.length > FILE_REPLACEMENTS_MAX_REPLACEMENTS_PER_RULE) {
        throw new Error(`Rule for "${rule.targetPath}" exceeds max replacements (${FILE_REPLACEMENTS_MAX_REPLACEMENTS_PER_RULE})`);
      }
      const validReplacements = [];
      for (const rep of rule.replacements) {
        if (typeof rep.pattern !== "string" || !rep.pattern) {
          throw new Error(`Invalid pattern in rule for "${rule.targetPath}"`);
        }
        if (typeof rep.replacement !== "string") {
          throw new Error(`Invalid replacement in rule for "${rule.targetPath}"`);
        }
        if (rep.replacement.length > FILE_REPLACEMENTS_MAX_REPLACEMENT_LENGTH) {
          throw new Error(`Replacement too long in rule for "${rule.targetPath}" (max ${FILE_REPLACEMENTS_MAX_REPLACEMENT_LENGTH})`);
        }
        const flags = rep.flags || "";
        if (!FILE_REPLACEMENTS_ALLOWED_FLAGS.test(flags)) {
          throw new Error(`Invalid flags "${flags}" in rule for "${rule.targetPath}" (allowed: i, m)`);
        }
        // Validate regex with g flag always applied
        if (safeRegex(rep.pattern, "g" + flags, FILE_REPLACEMENTS_MAX_PATTERN_LENGTH) === null) {
          throw new Error(`Invalid or unsafe regex pattern "${rep.pattern}" in rule for "${rule.targetPath}"`);
        }
      ...
      validated.push({ targetPath: rule.targetPath.trim(), replacements: validReplacements });
    }
    preset.fileReplacements = validated;
  }
  await writePresetJson(id, preset);
...
export async function applyPreset(id, instanceName, options = {}) {
  ...
  let backedUp = false;
  const added = { mods: [], configs: [], kubejs: [], resourcepacks: [], disabledMods: [], fileReplacements: [] };
  ...
  // 5. Disable mods matching disableMods patterns
  const disableModResults = [];
  if (preset.disableMods && preset.disableMods.length > 0) {
    try {
      const matches = await findMatchingMods(instanceModDir, preset.disableMods);
      for (const { fileName } of matches) {
        const srcPath = join(instanceModDir, fileName);
        const destPath = join(instanceModDir, `${fileName}.disabled`);
        try {
          await rename(srcPath, destPath);
          added.disabledMods.push(fileName);
          disableModResults.push({ fileName, success: true });
        } catch (err) {
          disableModResults.push({ fileName, success: false, error: err.message });
          errors.push(`Disable ${fileName}: ${err.message}`);
        }
      }
    } catch (err) {
      errors.push(`Disable mods: ${err.message}`);
    }
  }

  // 6. Apply file replacements (post-processing)
  const fileReplacementResults = [];
  const fileReplacements = preset.fileReplacements || [];
  for (const rule of fileReplacements) {
    const destPath = join(instancePath, rule.targetPath);

    // Path traversal check
    const frRel = relative(instancePath, destPath);
    if (frRel.startsWith("..") || isAbsolute(frRel)) {
      fileReplacementResults.push({
        targetPath: rule.targetPath,
        matchCount: 0,
        success: false,
        error: "Path escapes instance directory",
      });
      errors.push(`File replacement ${rule.targetPath}: path escapes instance directory`);
      continue;
    }
    ...
    if (!existsSync(destPath)) {
      ...
      errors.push(`File replacement ${rule.targetPath}: file not found`);
      continue;
    }
    ...
    try {
      ...
      if (backup) {
        const did = await backupFile(destPath, backupDir, join("fileReplacements", rule.targetPath));
        if (did) backedUp = true;
        added.fileReplacements.push(rule.targetPath);
      }

      let content = await readFile(destPath, "utf-8");
      let totalMatches = 0;

      for (const rep of rule.replacements) {
        const flags = "g" + (rep.flags || "");
        const re = safeRegex(rep.pattern, flags, FILE_REPLACEMENTS_MAX_PATTERN_LENGTH);
        if (!re) {
          errors.push(`File replacement ${rule.targetPath}: unsafe or invalid regex "${rep.pattern}"`);
          continue;
        }
        const matches = content.match(re);
        totalMatches += matches ? matches.length : 0;
        content = content.replace(re, rep.replacement);
      }

      await writeFile(destPath, content);
      fileReplacementResults.push({ targetPath: rule.targetPath, matchCount: totalMatches, success: true });
    } catch (err) {
      fileReplacementResults.push({
        targetPath: rule.targetPath,
        matchCount: 0,
      ...
      errors.push(`File replacement ${rule.targetPath}: ${err.message}`);
    }
  }
  ...
  return {
    ...
    disabledMods: disableModResults,
    fileReplacements: fileReplacementResults,
    errors,
    backedUp,
  };
}
...
export async function previewPreset(id, instanceName) {
  ...
  // Preview disabled mods
  let disabledMods = [];
  if (preset.disableMods && preset.disableMods.length > 0) {
    try {
      disabledMods = await findMatchingMods(instanceModDir, preset.disableMods);
    } catch {
      /* non-ENOENT errors already filtered by findMatchingMods */
    }
  }

  // Preview file replacements
  const fileReplacementPreviews = [];
  const fileReplacements = preset.fileReplacements || [];
  for (const rule of fileReplacements) {
    const destPath = join(instancePath, rule.targetPath);
    const frRel = relative(instancePath, destPath);
    if (frRel.startsWith("..") || isAbsolute(frRel)) continue;

    if (!existsSync(destPath)) {
      fileReplacementPreviews.push({
        targetPath: rule.targetPath,
        replacementCount: rule.replacements.length,
        totalMatches: 0,
        fileExists: false,
      });
      continue;
  ...
  return {
    ...
    disabledMods,
    fileReplacements: fileReplacementPreviews,
  };
}
...
export async function rollbackPreset(id, instanceName) {
  ...
  const errors = [];
  const counts = { mods: 0, configs: 0, kubejs: 0, resourcepacks: 0, removed: 0, reEnabled: 0 };
  ...
  // 2. Remove added files and re-enable disabled mods using backup manifest
  const manifestPath = join(backupDir, "manifest.json");
  if (existsSync(manifestPath)) {
    try {
      const manifest = JSON.parse(await readFile(manifestPath, "utf8"));
      if (manifest.added) {
        // Remove files that were added (not overwritten) during apply
        for (const cat of categories) {
        ...
        const disabledFiles = manifest.added.disabledMods;
        if (Array.isArray(disabledFiles)) {
          const instanceModDir = join(instancePath, "mods");
          for (const fileName of disabledFiles) {
            // Path traversal guard
            const modRel = relative(instanceModDir, join(instanceModDir, fileName));
            if (modRel.startsWith("..") || isAbsolute(modRel)) continue;
            const disabledPath = join(instanceModDir, `${fileName}.disabled`);
            const originalPath = join(instanceModDir, fileName);
            try {
              if (existsSync(disabledPath)) {
                await rename(disabledPath, originalPath);
                counts.reEnabled++;
              }
            } catch (err) {
              errors.push(`Re-enable ${fileName}: ${err.message}`);
            }
          }
        }
      }
