...
function presetCategoryDir(id, category) {
  switch (category) {
    case 'configs': return presetConfigsDir(id);
    case 'kubejs': return presetKubejsDir(id);
    case 'resourcepacks': return presetResourcepacksDir(id);
    default: throw new Error(`Unknown category: ${category}`);
  }
}

const TEXT_EXTENSIONS = new Set([
  '.js', '.json', '.json5', '.toml', '.cfg', '.conf', '.properties',
  '.txt', '.md', '.yml', '.yaml', '.xml', '.html', '.css', '.csv',
  '.lang', '.mcmeta', '.zs', '.snbt', '.mcfunction',
  '.fsh', '.vsh', '.ini', '.bat', '.sh', '.command',
]);
  ...
  async function walk(dir, prefix = '') {
    ...
    for (const item of items) {
      ...
      if (item.isDirectory()) {
        ...
        entries.push({ targetPath: relPath, sizeBytes: st.size, isText: isTextFile(item.name) });
      }
    }
  }
...
async function importCategoryFromFolder(id, category, folderPath, fileFilter = null, recurse = true) {
  ...
  async function walk(srcDir, relPrefix = '') {
    ...
    for (const item of items) {
      ...
      if (item.isDirectory()) {
        ...
        const destPath = join(catDir, relPath);
        await mkdir(dirname(destPath), { recursive: true });
        await cp(srcPath, destPath);
        const st = await stat(destPath);
        imported.push({ targetPath: relPath, sizeBytes: st.size, isText: isTextFile(item.name) });
      }
    }
  }
...
function sanitizeTargetPath(targetPath) {
  ...
  const parts = targetPath.split('/').filter(p => p && p !== '..' && p !== '.');
  if (parts.length === 0) {
...
function detectConfigTargetPath(filePath) {
  ...
  for (let i = segments.length - 1; i >= 0; i--) {
    if (segments[i].toLowerCase() === 'config') { configIdx = i; break; }
  }
  ...
  if (configIdx !== -1) {
    ...
    for (let i = 0; i < configIdx; i++) {
      if (segments[i].toLowerCase() === 'kubejs') {
        throw new Error('This appears to be a KubeJS config file. Use the KubeJS section instead.');
      }
    }
...
function detectKubejsTargetPath(filePath) {
  ...
  let kubejsIdx = -1;
  for (let i = segments.length - 1; i >= 0; i--) {
    if (segments[i].toLowerCase() === 'kubejs') { kubejsIdx = i; break; }
  }
...
export async function importSingleConfigFile(id, filePath) {
  return await importSingleFile(id, 'configs', filePath, detectConfigTargetPath);
}
...
export async function importResourcepacksFromFolder(id, folderPath) {
  ...
  return await importCategoryFromFolder(id, 'resourcepacks', folderPath, filter, false);
}
...
export async function applyPreset(id, instanceName) {
  ...
  for (const kjs of preset.kubejs) {
    ...
    if (kjsRel.startsWith('..') || isAbsolute(kjsRel)) {
      kubejsResults.push({ targetPath: kjs.targetPath, success: false, error: 'Path escapes instance directory' });
    ...
    try {
      ...
      kubejsResults.push({ targetPath: kjs.targetPath, success: false, error: err.message });
  ...
  for (const rp of preset.resourcepacks) {
    ...
    if (rpRel.startsWith('..') || isAbsolute(rpRel)) {
      resourcepackResults.push({ targetPath: rp.targetPath, success: false, error: 'Path escapes instance directory' });
      errors.push(`Resource pack ${rp.targetPath}: path escapes instance directory`);
      continue;
    }
    ...
    try {
      ...
      resourcepackResults.push({ targetPath: rp.targetPath, success: false, error: err.message });
